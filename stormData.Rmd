...
```{r severeDamage}
severeDmg <- select(damageData, EVTYPE, totalDamage) %>%
    filter(totalDamage >= 1E6)

severeFreq <- group_by(severeDmg, EVTYPE) %>%
    summarise(count = n()) %>%
    arrange(desc(count))

top5 <- head(severeFreq, 5)

breaks <- 10^(seq(6, 11, 0.5))
ticks <- 10^(6:10)
histplot <- ggplot(severeDmg, aes(x = totalDamage)) +
    geom_histogram(breaks = breaks) +
    scale_x_log10(breaks = ticks) +
    labs(title = "Histogram of Events with severe Damage",
         x = "Damage in USD",
         y = "Frequency")


top5$EVTYPE <- gsub(" ", "\n", top5$EVTYPE)
evplot <- ggplot(top5, aes(x = reorder(EVTYPE, -count), y = count)) +
    geom_bar(stat = "identity") +
    labs(title = "Most frequent Events with severe Damage",
         x = "",
         y = "Nr. of events")

grid.arrange(histplot, evplot, nrow = 2)
```
